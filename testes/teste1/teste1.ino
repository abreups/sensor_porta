/*===============================================================================================
  ...
  teste2: envia pacote com alarmes desconhecidos.
...
#if OLED > 0
    display.drawString(0, 10, "Inicialização LoRa falhou");
    display.display();                  
#endif
...
#endif
  // transmite um pacote com alarme desconhecido
  if(portaAberta) {
      // prepara mensagem de alarme que porta foi aberta
      S.Add( RESP );
      S.Add( SLAVEID );
      S.Add( 0x99 );				// 0x99 é um tipo de alarme não definido
      S.Add( PORTAABERTA );
